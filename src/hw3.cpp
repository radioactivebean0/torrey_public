        ...
        for (y = y0; y < y1; ++y){
            for (x = x0; x < x1; ++x){
                ...
                for (sample = 0; sample < spp; ++sample){
                    ...
                    if (hit_bvh(scene.bvh, ray, scene.camera.lookfrom, eps, eps, infinity<Real>(), &hs, t, uv, true)){ // was a hit
                        pt = scene.camera.lookfrom + t * ray;
                        color = get_color(scene, ray, pt, eps, hs, uv, true, true);
                    } else {
                        color = scene.background_color;
                    }
                    color_sum += color;
        ...
        for (y = y0; y < y1; ++y){
            for (x = x0; x < x1; ++x){
                ...
                for (sample = 0; sample < spp; ++sample){
                    ...
                    if (hit_bvh(scene.bvh, ray, scene.camera.lookfrom, eps, eps, infinity<Real>(), &hs, t, uv, true)){ // was a hit
                        pt = scene.camera.lookfrom + t * ray;
                        color = get_color(scene, ray, pt, eps, hs, uv, true, true, true);
                    } else {
                        color = scene.background_color;
                    }
                    color_sum += color;
        ...
        for (y = y0; y < y1; ++y){
            for (x = x0; x < x1; ++x){
                ...
                for (sample = 0; sample < spp; ++sample){
                    ...
                    if (hit_bvh(scene.bvh, ray, scene.camera.lookfrom, eps, eps, infinity<Real>(), &hs, t, uv, true)){ // was a hit
                        pt = scene.camera.lookfrom + t * ray;
                        color = radiance(scene, ray, pt, eps, hs, uv, true, true, true, pcg_state);
                    } else {
                        color = scene.background_color;
                    }
                    color_sum += color;
...
Image3 hw_3_f(const std::vector<std::string> &params) {
    // Homework 3.4: area lights
    Timer timer;
    ParsedScene pscene = parse_scene(params[0]);
    std::cout << "Scene parsing done. Took " << tick(timer) << " seconds." << std::endl;
