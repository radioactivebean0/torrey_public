...
Scene::Scene(const ParsedScene &scene) :
    ...
    for (const ParsedMaterial &parsed_mat : scene.materials) {
        if (auto *diffuse = std::get_if<ParsedDiffuse>(&parsed_mat)) {
            ...
            materials.push_back(Material{.material_type = material_e::DiffuseType, .reflectance = std::get<Vector3>(diffuse->reflectance)});
        } else if (auto *mirror = std::get_if<ParsedMirror>(&parsed_mat)) {
            // We assume the reflectance is always RGB for now.
            materials.push_back(Material{.material_type = material_e::MirrorType, .reflectance = std::get<Vector3>(mirror->reflectance)});
        } else {
            assert(false);
        }
    }
...
bool closest_hit(const std::vector<Shape> &shapes, const Vector3 &ray, const Vector3 &ray_origin, const Real eps, int &obj_id, Real &t_val, Vector3 &b_coord){
    t_val = -1.0;
    obj_id = -1;
    int shape;
    Real t_temp;
    ...
    for (shape = 0; shape < shapes.size(); ++shape){
        if (auto *sph = std::get_if<Sphere>(&shapes.at(shape))){ // sphere intersection test
            ...
            if (get_tri_intersect(p0, p1, p2, ray, ray_origin, eps, intersection, t_temp)){
                if (t_temp >= 0.0 && (t_val < 0 || t_val > t_temp) && distance(ray_origin, ray_origin + ray * t_temp)> eps){ // new closest obj
                    t_val = t_temp;
                    obj_id = shape;
                    b_coord = intersection;
                }
            }
        }
    }
...
Image3 hw_2_1(const std::vector<std::string> &params) {
    ...
    for (int y = 0; y < img.height; y++) {
        for (int x = 0; x < img.width; x++) {
            ...
            Real t_val;
            if (get_tri_intersect(p0, p1, p2, ray, camera.lookfrom, eps, intersection, t_val)){
                img(x,y) = intersection;
            } else {
...
Image3 hw_2_2(const std::vector<std::string> &params) {
    ...
    for (int y = 0; y < img.height; y++) {
        for (int x = 0; x < img.width; x++) {
            Vector3 ray = get_ray(camera, x, y, pcg_state);
            Vector3 close_intersect;
            Real dist = infinity<Real>();
            bool hit = false;
            Real t_val;
            for (int triangle = 0; triangle < indices.size(); triangle++){
                Vector3 intersection;
                if (get_tri_intersect(positions.at(indices.at(triangle).x),positions.at(indices.at(triangle).y), positions.at(indices.at(triangle).z), ray, camera.lookfrom, eps, intersection, t_val)){
                    hit = true;
                    if (distance(camera.lookfrom + t_val * ray, camera.lookfrom) < dist){
                        close_intersect = intersection;
                        dist = distance(camera.lookfrom + t_val * ray, camera.lookfrom);
                    }
                }
            }
            if (hit){
                img(x,y) = close_intersect;
            } else {
                img(x,y) = Vector3{0.5,0.5,0.5};
            }
        }
        ...
        for (y = y0; y < y1; ++y){
            for (x = x0; x < x1; ++x){
                ...
                for (sample = 0; sample < spp; ++sample){
                    ...
                    obj_id = -1;
                    t = -1.0;
                    if (closest_hit(scene.shapes, ray, scene.camera.lookfrom, eps, obj_id, t, b_coord)){ // was a hit
                        pt = scene.camera.lookfrom + t * ray;
                        color = get_color(scene, ray, pt, eps, obj_id, false, false);
                    } else {
        ...
        for (y = y0; y < y1; ++y){
            for (x = x0; x < x1; ++x){
                ...
                for (sample = 0; sample < spp; ++sample){
                    ray = get_ray(scene.camera, x, y, pcg_state);
                    t = -1.0;
                    if (hit_bvh(scene.bvh, ray, scene.camera.lookfrom, eps,  &hit_shape, t, b_coord, false)){ // was a hit
                        pt = scene.camera.lookfrom + t * ray;
                        color = get_color(scene, ray, pt, eps, hit_shape, true, false);
                    } else {
                        color = scene.background_color;
                    }
                    color_sum += color;
        ...
        for (y = y0; y < y1; ++y){
            for (x = x0; x < x1; ++x){
                ...
                for (sample = 0; sample < spp; ++sample){
                    ray = get_ray(scene.camera, x, y, pcg_state);
                    t = -1.0;
                    if (hit_bvh(scene.bvh, ray, scene.camera.lookfrom, eps,  &hit_shape, t, b_coord, true)){ // was a hit
                        pt = scene.camera.lookfrom + t * ray;
                        color = get_color(scene, ray, pt, eps, hit_shape, true, true);
                    } else {
                        color = scene.background_color;
                    }
                    color_sum += color;
